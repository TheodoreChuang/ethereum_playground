...
function App() {
  ...
  async function withdrawEth(e) {
    ...
    await contract.methods.withdrawEth(to, amount).send({ from: accounts[0] });
  }
  ...
  async function transferShares(e) {
    ...
    await contract.methods.transferShares(amount, recipient).send({ from: accounts[0] });
    await updateShares();
  }
                ...
                <tr key={proposal.id}>
                  ...
                  <td>
                    {isFinished(proposal) ? (
                      ...
                      <button onClick={e => vote(proposal.id)} type="submit" className="btn btn-primary">
                        Vote
                      </button>
                    )}
                  </td>
                  <td>{new Date(parseInt(proposal.end) * 1000).toLocaleString()}</td>
                  <td>
                    {proposal.executed ? (
                      "Yes"
                    ) : admin.toLowerCase() === accounts[0]?.toLowerCase() ? (
                      <button onClick={e => executeProposal(proposal.id)} type="submit" className="btn btn-primary">
                        Execute
                      </button>
                    ) : (
                      "No"
